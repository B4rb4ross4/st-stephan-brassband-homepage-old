...
apsinth.ApsinthModule=Modul.extend({CONFIG_MIN_WIDTH:300,proxy:null,static_proxy:null,handler_proxy:null,view_proxy:null,configSaveHandler:"config",mainView:"main",data:null,data_admin:null,configIsOpen:false,appendError:false,errorTarget:"auto",fieldErrorsTarget:"auto",loadMainViewAfterSave:true,ajaxUtil:null,initialize:function(e,d,b,g){try{apsinth.util.EventingMixin.mixin(this);
...
}c=e.createElement(a);c.id=f;c.src="//connect.facebook.net/"+(FacebookModule.msg.all&&FacebookModule.msg.all.facebookLocale?FacebookModule.msg.all.facebookLocale:apsinth.lang)+"/all.js#xfbml=1";
...
}}FB.Event.subscribe("xfbml.render",function(){var e={width:a+"px",height:d+"px"};b.find("iframe").add(b).css(e);
c.find("iframe").add(c).css("width",a+"px")})}if(this.mode=="admin"&&window.FB){FB.XFBML.parse(this.getMainJQ().get(0))
...
this.disableToggleInit("badge","badge");jQuery(".wrapper-color",a).diy_combobox({editable:false});a.find(".form-item input.checkbox").diy_checkbox();
...
b=false}}}return b},getConfigData:function(){var a=this.getConfigJQ();return this.settings},_initMainView:function(){var d=this.getMainJQ();
var c=d.find(".twtr-widget-base");var a=d.find(".twitter-tweet-button");var b=this;if(Twitter.widgetJsState===0){Twitter.widgetJsState=1;
var f="//platform.twitter.com/widgets.js";b.initWidget(c[0]);var e=function(){Twitter.widgetJsState=2;
jQuery(window).trigger("twitter-widget-js-loaded")};jQuery.xLazyLoader({js:[f],success:e})}else{if(Twitter.widgetJsState===1){jQuery(window).one("twitter-widget-js-loaded",function(){setTimeout(function(){b.initWidget(c[0]);
if(twttr&&twttr.widgets){twttr.widgets.load()}},500)})}else{b.initWidget(c[0]);if(twttr&&twttr.widgets){twttr.widgets.load()
...
}jQuery(f).removeAttr("disabled")}}})}},onDelete:function(){}});Twitter.widgetJsState=0;TWTR1and1=window.TWTR1and1||{};
if(!Array.forEach){Array.prototype.filter=function(f,g){var e=g||window;var b=[];for(var d=0,c=this.length;
d<c;++d){if(!f.call(e,this[d],d,this)){continue}b.push(this[d])}return b};Array.prototype.indexOf=function(b,c){var c=c||0;
for(var a=0;a<this.length;++a){if(this[a]===b){return a}}return -1}}(function(){if(TWTR1and1&&TWTR1and1.Widget){return
...
apsinth.version='branches/release-sprint57-';apsinth.debug=false;
